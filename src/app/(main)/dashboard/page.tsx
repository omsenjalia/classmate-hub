...
export default function DashboardPage() {
  ...
  const displaySubjects = subjects.length > 0
    ? subjects.slice(0, 6).map((subject) => ({ ...subject, href: `/materials?subject=${encodeURIComponent(subject.id)}` }))
    : SYLLABUS_PDFS.map((subject) => ({ ...subject, href: `/materials?search=${encodeURIComponent(subject.code)}` }))
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <Layers3 className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /> Browse by Subject
              </h2>
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl bg-indigo-100 dark:bg-indigo-500/15 border border-indigo-200 dark:border-indigo-500/30 flex items-center justify-center">
                        <BookOpen className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                      </div>
                      <div>
                        <p className="text-xs font-mono uppercase tracking-wider text-indigo-600 dark:text-indigo-400">{subject.code}</p>
                        <h3 className="mt-1 text-base font-bold text-gray-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                          {subject.name}
                        </h3>
                      </div>
                    </div>
